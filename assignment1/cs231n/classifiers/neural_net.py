...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    softmax_val = np.exp(scores) / np.sum(np.exp(scores), axis = 1).reshape(-1,1)
    loss = -np.sum(np.log(softmax_val[range(N), list(y)]))
    loss /= N
    ...
    ds[range(N), list(y)] -= 1
    ds /= N
    grads['W2'] = hidden_output.T.dot(ds) + 2 * reg * W2
